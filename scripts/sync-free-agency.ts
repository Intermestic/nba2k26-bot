...
const scrapedData = JSON.parse(fs.readFileSync("/home/ubuntu/scrape_free_agency_ratings.json", "utf-8"));
...
async function main() {
  console.log("Starting free agency ratings sync...\n");
  ...
  const unmatched: { name: string; overall: number }[] = [];
  
  // Process free agency
  const teamResult = scrapedData.results[0];
  const scrapedPlayers: ScrapedPlayer[] = JSON.parse(teamResult.output.players_json);
  
  console.log(`=== FREE AGENCY (${scrapedPlayers.length} players) ===\n`);
  ...
  for (const scraped of scrapedPlayers) {
    ...
    if (match) {
      ...
      unmatched.push({ name: scraped.name, overall: scraped.overall });
