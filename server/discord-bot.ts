        ...
        try {
          ...
          if (rows.length === 0) {
            ...
            try {
              ...
              `);
              console.log(`[Discord Bot] Successfully recreated lock`);
              ...
              if (isDuplicateEntry) {
                ...
                try {
                  await db.execute(sql`
                    ...
                    WHERE id = 1 AND expiresAt < NOW()
                  `);
                  console.log(`[Discord Bot] Took ownership of expired lock`);
                  consecutiveZeroRows = 0;
                  lockRefreshFailures = 0;
                  return;
                } catch (updateError) {
            ...
            if (lockExpiry < new Date()) {
              ...
              try {
                ...
                await db.execute(sql`
                  UPDATE bot_instance_lock
                  SET instanceId = ${INSTANCE_ID}, expiresAt = FROM_UNIXTIME(${expiryTimestamp}), lockedAt = NOW()
                  WHERE id = 1 AND instanceId = ${rows[0].instanceId}
                `);
                console.log(`[Discord Bot] Successfully took over expired lock`);
                consecutiveZeroRows = 0;
                lockRefreshFailures = 0;
                return;
              } catch (takeoverError) {
                console.error('[Discord Bot] Failed to take over expired lock:', takeoverError);
              }
            }
          }
        } catch (debugError) {
